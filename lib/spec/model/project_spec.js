...
describe('model', function () {
    describe('project', function () {
        ...
        describe('update priority', function () {
            describe('with moving first after second (i.e. swapping first for second)', function () {
                var firstId, secondId;
                beforeEach(function (done) {
                    firstId = project.issues[0]._id;
                    secondId = project.issues[1]._id;
                    project.updateIssuePriority(firstId, 1, ew(function () { reFindProject(done); }));
                });

                it('first issue expected to be old second', function () { should(project.issues[0]._id).be.eql(secondId); });
                it('second issue expected to be old first', function () { should(project.issues[1]._id).be.eql(firstId); });
            });
        });
    });
});