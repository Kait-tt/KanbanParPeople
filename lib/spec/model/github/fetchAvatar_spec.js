...
describe('model', function () {
    describe('github', function () {
        describe('#fetchAvatar', function () {
            ...
            it('first item is start with http', function () { should(res[0].url).startWith('http'); });
            it('second item is path', function () {
                should(res[0].path).containEql('public');
                should(res[0].path).containEql('images');
                should(res[0].path).containEql('avatar');
                should(res[0].path).containEql(username + '.');
            });
        });
    });
});