...
GitHub.prototype.attachLabel = function (repo, user, issue, label, callback) {
    this.octo.post('/repos/' + user + '/' + repo + '/issues/' + issue.github.number + '/labels', [label.name],
        function(err, status, body, header) {
            if (err) { return callback(err); }
            if (status !== 200) { console.error(err); return callback(new Error('Attach label error')); }
...
GitHub.prototype.detachLabel = function (repo, user, issue, label, callback) {
    this.octo.del('/repos/' + user + '/' + repo + '/issues/' + issue.github.number + '/labels/' + label.name, {},
        function(err, status, body, header) {
            if (err && err.message === 'Label does not exist') { callback(null, body, header); }
            if (err) { return callback(err); }
            if (status !== 204 && status !== 200) { console.error(err); return callback(new Error('Detach label error')); }
            return callback(null, body, header);
        });
};
...
GitHub.prototype.fetchUserAvatar = function (usernames, callback) {
    ...
    async.map(usernames, function (username, next) {
        github.api.users.getForUser({user: username}, function (err, res) {
            if (err && err.code !== 404) { return next(err); }
            if (err && err.code === 404) { return next(null, {url: null, path: null}); }
        ...
        if (err) { console.error(err); return callback(new Error(err)); }
    ...
    async.parallel({
        ...
        members: function (next) {
            api.repos.getCollaborators({
                ...
                if (err) { next(err); }
                else { next(null, _.map(data, 'login')); }
            });
        },
...
function getIssues(api, o, callback) {
    ...
    api.issues.getForRepo(o, function (err, data) {
        if (err) { callback(err); return; }
