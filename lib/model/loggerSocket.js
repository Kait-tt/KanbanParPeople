...
LoggerSocket.prototype.loggingOnEvent = function (key, req) {
    logger.info('on ' + key + ': ' + JSON.stringify({username: this.username, projectId: this.projectId, req: req}));
    Log.logging({type: 'socket', action: 'on', key: key, socketId: this.socket.id, username: this.username, projectId: this.projectId, req: req});
};

LoggerSocket.prototype.loggingOnCallback = function (key, res) {
    logger.info('callback ' + key + ': ' + JSON.stringify({username: this.username, projectId: this.projectId, res: res}));
    Log.logging({type: 'socket', action: 'callback', key: key, socketId: this.socket.id, username: this.username, projectId: this.projectId, res: res});
};
...
LoggerIO.prototype.loggingEmit = function (io, key, req) {
    ...
    try {
        ...
        Log.logging({type: 'socket', action: 'emit', key: key, projectId: projectId, req: req});
    } catch (e) { }
};
