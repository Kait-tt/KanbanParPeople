...
LoggerSocket.prototype.loggingOnEvent = function (key, req) {
    var params = {type: 'socket', action: 'on', key: key, socketId: this.socket.id, username: this.username, projectId: this.projectId, req: req};
    logger.info('on ' + key + ': ' + JSON.stringify(params));
    Log.logging(params);
};

LoggerSocket.prototype.loggingOnCallback = function (key, res) {
    var params = {type: 'socket', action: 'callback', key: key, socketId: this.socket.id, username: this.username, projectId: this.projectId, res: res};
