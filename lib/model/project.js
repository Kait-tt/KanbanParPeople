...
Projects.static('addIssue', function (projectWhere, issueParams, callback) {
    this.findOne(projectWhere, function (err, project) {
        if (err) { callback(err); return; }
        if (!project) { callback(new Error('undefined project')); return; }

        project.addIssue(issueParams, callback);
    });
});
...
Projects.static('removeIssue', function (projectWhere, issueId, callback) {
    this.findOne(projectWhere, function (err, project) {
        if (err) { callback(err); return; }
        if (!project) { callback(new Error('undefined project')); return; }

        project.removeIssue(issueId, callback);
    });
});
/*** /issue ***/
