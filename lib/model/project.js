...
Projects.method('updateStage', function (token, issueId, toStage, userId, callback) {
    ...
    if (GitHubHook.isHooked(project) && String(userId) !== String(issue.assignee)) { // assignee with github
        var github = new GitHubHook(token);
        github.assignIssue(project, {issue: issue, userId: userId}, function(err, res) {
            if (err) { return callback(err); }
            // update stage
            issue.stage = toStage;
