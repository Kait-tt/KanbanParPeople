...
Projects.pre('save', function (next) {
    var labelNames = _.pluck(this.labels, 'name');
...
Projects.pre('save', function (next) {
    var labelIds = _.chain(this.labels)
        .map(function (label) { return [label._id, true]; })
        .zipObject()
        .value;

    this.issues.forEach(function (issue) {
        if (issue.labels) {
            if (issue.labels.some(function (label) { return !labelIds[label._id]; })) {
                throw new Error('required label attached to issue is included ');
            }
        }
    });
