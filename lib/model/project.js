...
Projects.method('updateIssuePriority', function (issueId, insertBeforeOfIssueId, callback) {
    var project = this;

    var targetIssue = project.findIssueById(issueId);
    if (!targetIssue) { return callback(new Error('undefined issue id: ' + issueId)); }
    var beforePos = project.issues.indexOf(targetIssue);
    ...
    // remove
    project.issues.splice(beforePos, 1);

    // insert
    if (!insertBeforeOfIssue) {
        project.issues.push(targetIssue);
    } else {
        project.issues.splice(project.issues.indexOf(insertBeforeOfIssue), 0, targetIssue);
    }

    project.save(function (err, project) {
        if (err) { callback(err); return; }
        callback(err, project, targetIssue, insertBeforeOfIssue);
    });
});
