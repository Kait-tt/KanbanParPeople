...
Projects.method('addIssue', function (token, params, callback) {
    ...
    if (token && project.github && project.github.sync) {
        (new GitHub(token)).createIssue(project.github.repoName, project.github.userName, params, callback2);
    } else {
        Issue.create(params, callback2);
    }

    function callback2(err, issue) {
        if (err) { callback(err); return; }
...
Projects.method('removeIssue', function (token, issueId, callback) {
    ...
    if (token && project.github && project.github.sync) {
        (new GitHub(token)).closeIssue(project.github.repoName, project.github.userName, issue, callback2);
    } else {
        callback2(null);
    }
...
Projects.method('updateIssueDetail', function (issueId, title, body, callback) {
    var project = this;
